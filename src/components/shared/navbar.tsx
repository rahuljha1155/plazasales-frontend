...
export default function Navbar() {
  ...
  const { brands, fetchBrands, getBrandBySlug, productBrandLogoUrl } =
    useBrandStore();
  ...
  const isBrandProductPage =
    pathname.startsWith("/brand/") && !pathname.includes("/forward");
  const showHeader =
    pathname.startsWith("/brand/") || pathname.startsWith("/products/");
  const navigation = [
    {
      title: "Home",
      path: "/",
      ...
      >
        <div className="items-center justify-between lg:py-1 gap-x-4 lg:gap-x-8 xl:gap-x-14 px-4  xl:px-0 lg:max-w-7xl mx-auto lg:flex">
          <div className="flex items-center justify-between w-full  lg:w-auto py-2">
            ...
            <TransitionLink
              href="/"
              className="pointer-events-auto  w-fit  flex justify-center items-center "
            >
              <Image
                src={logo}
                width={170}
                height={100}
                alt="Plaza sales"
                className="h-6 sm:h-8 w-auto  dark:invert-50 object-contain object-center lg:object-left"
              />
            </TransitionLink>
          ...
          <div
            className={`nav-menu flex-1 w-full max-w-2xl pb-3 mt-4 lg:block md:pb-0 md:mt-0 ${state ? "block" : "hidden"}`}
          >
                  ...
                  {item.isDrapdown ? (
                    <TransitionLink href={item?.path}>
                      ...
                      >
                        {item.title}
                        <Icon
                          icon="famicons:chevron-up"
                          className={`w-5 h-5 transition-all duration-300 ${drapdownState.idx === idx && drapdownState.isActive ? "" : "rotate-180"}`}
                        />
                      </button>
                    </TransitionLink>
                  ) : (
            ...
            >
              <div className="flex gap-2 items-center group text-zinc-500 border rounded-full px-4 py-1.5  hover:text-white transition-all cursor-pointer hover:bg-linear-to-r from-green-600 to-green-800">
                <Icon
                  ...
                  className="size-4  group-hover:grayscale-0"
                />
                Whatsapp
              </div>
            </TransitionLink>
      ...
      {showSearchModal && (
        ...
        >
          ...
          >
            <div className=" border-b border-zinc-200 dark:border-zinc-700">
              <div className="flex p-4 items-center gap-3">
                <Icon
                  ...
                  className="size-5 md:size-6 text-zinc-400 shrink-0"
                />
            ...
            {/* Search Results */}
            <ScrollArea
              className="h-[calc(100vh-70px)] md:h-[calc(70vh-70px)]"
              style={
                {
                  contain: "layout style paint",
                  touchAction: "pan-y",
              ...
              {isSearching ? (
                ...
                searchResults.length > 0 ? (
                  <div className="divide-y divide-zinc-100 dark:divide-zinc-800 p-2">
                    {searchResults.map((product) => {
                      ...
                      if (!hasValidUrl) {
                        console.warn(
                          "Search result missing URL data:",
                          product,
                        );
                        return null;
                      }
                      ...
                      return (
                        ...
                        >
                          ...
                          <Icon
                            ...
                            className="size-5 text-zinc-400 group-hover:text-primary transition-colors shrink-0"
                          />
                        </TransitionLink>
                      );
                    })}
                  ...
                  <div className="p-12 md:p-16 text-center">
                    <Icon
                      ...
                      className="size-16 md:size-20 mx-auto mb-4 text-zinc-300 dark:text-zinc-600"
                    />
                ...
                <div className="p-4  ">
                  ...
                  <div className="flex gap-1  mt-2 items-center flex-wrap">
                    {brands?.map((brand) => {
                      if (brand?.name.toLowerCase().includes("forward"))
                        return null;
                      return (
                        ...
                        <TransitionLink
                          ...
                          onClick={closeSearchModal}
                          href={`/products`}
                          className="bg-muted/80 hover:text-white text-zinc-500 hover:bg-primary px-4 py-1 flex gap-1 items-center rounded-sm"
                        >
                          {brand?.name}{" "}
                        </TransitionLink>
                      );
